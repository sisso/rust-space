...
pub fn apply_damages(components: &Components, logs: &mut Vec<CombatLog>, ships: &mut HashMap<ShipInstanceId, &mut ShipInstance>, damages: Vec<DamageToApply>) {
...
fn apply_damage(components: &Components, logs: &mut Vec<CombatLog>, ship: &mut ShipInstance, damage: DamageToApply) {
...
fn generate_damage_indexes(damage_type: WeaponDamageType, amount: Damage, index: ArmorIndex, armor_width: u32) -> Vec<ArmorIndex> {
    match damage_type {
        WeaponDamageType::Penetration => generate_penetration_damage_indexes(amount, index,armor_width),
        WeaponDamageType::Explosive => generate_explosive_damage_indexes(amount, index,armor_width),
    }
}
